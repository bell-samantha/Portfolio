## Samantha Lynn Bell DHD 01/2022 ##
...
-Checks for superseded, canceled, or "not a case" cases, with the option for removal from analysis
...
-Assign a fatality rate to each Combined Disease Date, to allow for plotting of fatality rate over time by sex
...
```{r packages}
# unload any competing packages  (Error if none currently loaded)
...
if(dir.exists("T:/Epidemiology")){
  location <- "T:/Epidemiology/Data/MDSS/original/COVD-19 Coronavirus/2020_03_Coronavirus"
}
...
if(dir.exists(paste0("C:/Users/", Sys.info()[['login']], "/Box/COVID-19/COVID-19/Shared output/High Risk Facilities"))){
  highriskbox<- paste0("C:/Users/", Sys.info()[['login']], "/Box/COVID-19/COVID-19/Shared output/High Risk Facilities")}
...
if(dir.exists(paste0("C:/Users/", Sys.info()[['login']], "/Box/Data Portal Files - Public Facing"))){
  dataPortalFilesBox <- paste0("C:/Users/", Sys.info()[['login']], "/Box/Data Portal Files - Public Facing")}
...
methodLoad <- dlg_list(title = "Select the method of MDSS file loading", choices = c("2 or more new unique downloads", "multiple old unique split downloads + a new shorter download", "1 day-old full download + 1 new 14 day download", "1 complete download", "1 old complete download + 2 or more new shorter downloads"), gui = .GUI)$res
...
if(methodLoad=="multiple old unique split downloads + a new shorter download"){ # 2 old unique downloads & 1 new 90 day download
  # Use the svDialogs package to have a popup ask the user for the file 
  ...
  loaded <-  lapply(filepaths, function(x)read.csv(x, header=TRUE))
  #Make a blank tibble with the column names 
  dlg_message("Select the 1 new shorter file when prompted", type = "ok")
...
if(methodLoad=="1 old complete download + 2 or more new shorter downloads"){ # 2 or more new unique files plus one old full download
  ...
  # Load the new shorter downloads
    # Use the svDialogs package to have a popup ask the user for the file 
  inputFiles <- dlg_input("How many split download files do you have to load? (enter a number):")$res
  dlg_message("Select each file when prompted", type = "ok")
  for(i in 1:inputFiles){
    inputFilePath[i] <- dlg_open(location)$res
  }
...
covidDataDup <- suppressWarnings(covidDataDup %>% mutate_at(all_of(dateCols), funs(anydate(.)))) # origin adjusted due to weird loading
```
...
- Make a column with simple 5 digit investigation zip code, excluding test sites and faulty addresses and imputed zips from MDSS
...
covidDataDup <- covidDataDup %>% dplyr::rename(
  ...
  Was_quarantined_at_onset = At_time_of_symptom_onset__or_) %>% mutate( 
    ...
    Age_Units == "Years" | Age_Units == "years" ~ Patient_Age),# Otherwise keep the age (in years already)
      ...
      TRUE ~ "Alive"),
    Simple_Investigation_Zip = case_when( #Used to get consistent 5 digit zip code for all cases. Not in exclusion addresses, and not imputed by MDSS
      !grepl(exclusion_Addresses, Street_Address, ignore.case = TRUE)&!is.na(Patient_Address_Only) ~ substr(covidDataDup$Zip, 1, 5),
      grepl(exclusion_Addresses, Street_Address, ignore.case = TRUE)|is.na(Patient_Address_Only) ~ "NONE"),
...
# Find Patient Ids that occur more than once, and see if they are repeats or different testing dates
t2 <- table(covidData$Patient_ID)>1
w2 <- which(t2)
# Print the records with a comboID match (potential duplicates) or patientID match (true match)
dups2 <- covidData %>% filter( covidData$Patient_ID%in%names(w2) & newID!=0) %>% 
  group_by(comboID, newID, Referral_Date, Num_Occur, InvestigationID, Patient_ID, Case_Status, Investigation_Status, Deceased) %>% summarise()

test <- table((dups2%>%filter(Deceased=="Deceased"))$newID)
anyMissed <- which(test>1)
cat("If you missed any duplicates, their 'newID' will be listed here:  ")
(names(anyMissed))
...
covidData <- covidData %>% mutate(FirstDate = pmin(Referral_Date, Onset_Date, na.rm = TRUE ))
  ...
  CAPTURE all cases with same PatientID, order them by the new combo date created above
    FOR this record and the previously dated record:
			...
			IF there is a prior record for this same patient ID (before this one chronologically)
			  IF this date â previous date <=90
				  FLAG for deduplication
				  CDD = NA

   	    IF this date â previous date >90
		      CAPTURE the oldest test date that is > 90 days after the previous recordâs referral/onset date
			  	  (Donât want to pull test dates from previous cases this patient has)
...
suppressWarnings(for(i in 1:dim(covidData)[1]){
...
# Clean up the test date field for reporting
covidData <- covidData %>% mutate(
  PositiveCovidTestReportDates = gsub("\\)|c\\(", "", PositiveCovidTestReportDates ), 
  PositiveCovidTestDates = gsub("\\)|c\\(", "", PositiveCovidTestDates )
) 

covidData$oldestPositiveCovidTestDate <- suppressWarnings(anydate(sapply(covidData$PositiveCovidTestDates, 
                                                        function(x)(min(days(str_split(x, ",", simplify = TRUE))+anydate("1970-01-01"))))))
covidData$newestPositiveCovidTestDate <-  suppressWarnings(anydate(sapply(covidData$PositiveCovidTestDates, 
                                                        function(x)(max(days(str_split(x, ",", simplify = TRUE))+anydate("1970-01-01"))))))
covidData$newestPositiveCovidTestReport <-  suppressWarnings(anydate(sapply(covidData$PositiveCovidTestReportDates, 
                                                          function(x)(max(days(str_split(x, ",", simplify = TRUE))+anydate("1970-01-01"))))))
covidData$oldestPositiveCovidTestReport <-  suppressWarnings(anydate(sapply(covidData$PositiveCovidTestReportDates,  
                                                          function(x)(min(days(str_split(x, ",", simplify = TRUE))+anydate("1970-01-01"))))))
...
The updated CSTE definition is as follows:
  â At least two of the following:
...
â Rate[n] = {((C[1]/P[1])*100k)*W[1]} + {((C[2]/P[2])*100k)*W[2]} + ... 
where:
  C = # cases for said demographic
  P = census count for said demographic
...
For n of all unique Age Ranges from Age_Range variable
â Rate[n] = {((D[1]/P[1])*100k)*W[1]} + {((D[2]/P[2])*100k)*W[2]} + ... 
where:
...
zipSummary <- covidData %>% filter(!is.na(Patient_Address_Only)) %>% group_by(Simple_Investigation_Zip, Deceased) %>% summarise(cases = n()) %>% # summarize counts alive vs dead
  mutate(Zip_death_count_All = sum(cases[Deceased=="Deceased"])) %>% # tally the deaths
  mutate(Zip_case_count_All = Zip_death_count_All+cases) %>% # tally the total cases alive + dead
  filter(Deceased == "Alive") %>% group_by(Simple_Investigation_Zip, Zip_case_count_All, Zip_death_count_All) %>% summarise() # clean up the rows to one row per zip
...
zipPopTable <- left_join(zipPopTable, zipSummary, by = c("Zip_Code_2010" ="Simple_Investigation_Zip")) %>% mutate(
...
## Calculate TOTAL case and death rates for each zip code
zipPopTable$Zip_case_rate_All <- (zipPopTable$Zip_case_count_All/zipPopTable$zip_total_clipped)*100000
zipPopTable$Zip_death_rate_All <- (zipPopTable$Zip_death_count_All/zipPopTable$zip_total_clipped)*100000
...
# Save a non-suppressed version
write.xlsx(zipPopTable, paste0(location, "/For internal Tableau reporting/", "Zip_Rates.xlsx"), overwrite = T) 
write.xlsx(zipPopTable, paste0(outputLocation, "/", Sys.Date(), "_Full Zip_Rates.xlsx"), overwrite = T) 

# Make a special file for the internal dashboard
mayorDashZips <- zipPopTable %>% rename(
  zips = Zip_Code_2010,
  `covidData$Today[1]` = date_updated 
)
...
# The user is prompted for optional printing of the file if it is a Tuesday within the first 8 days of the month (to account for potential missed 1st Monday due to holiday or other analysis issues â The first Monday may have been day 1-7, and if missed the first Tuesday following the first Monday may be up to day 8.)
```{r publicZipFile}
# Make special file for the data portal
dataPortalRates <- zipPopTable %>% select(Zip_Code_2010, zip_total_clipped, Zip_case_count_All, Zip_death_count_All, Zip_case_rate_All, Zip_death_rate_All, date_updated) %>% rename(
  ZIP_Code = Zip_Code_2010,
  Clipped_ZIP_Population = zip_total_clipped,
  ZIP_Case_Count = Zip_case_count_All,
  ZIP_Death_Count = Zip_death_count_All,
...
if(weekday=="Monday" & d<=7){
  write.csv(dataPortalRates, paste0(dataPortalFilesBox, "/", "Detroit City COVID Confirmed Cases and Rates by ZIP Code.csv"),row.names=FALSE)
  write.xlsx(zipPopTable, paste0(location, "/PUBLIC DASHBOARD FILES/", "Zip_Rates.xlsx"), overwrite=T) 
...
if(weekday=="Tuesday" & d<=8 & d>1){
  did_print <- dlg_input(
    message = "It is the beginning of the month, did you already update the zip rates file for both the public dashboard and public data portal (yes or no)?\n[This is automatically done if the code is run on the 1st Monday of the month]", type = "yesno")$res
...
occupations_short <- covidData %>% filter(!is.na(Occupation) & anydate(Referral_Date)>Sys.Date()-90) %>% select(InvestigationID, Occupation)  #  short version 
```

** Save files all at once to the main dated analysis folder **
```{r saveInBulk  ## Samantha Lynn Bell DHD 2021 ##}
  c <- dim(covidData)[1]
  d <- dim(covidData%>%filter(Deceased=="Deceased"))[1]
  message2 <- capture.output(cat("The total case count is", c, " and the total death count is", d, ". Click 'ok' when ready to continue."))
...
# The data to save
dataToPrint <- list(
  pop_table_all, zipPopTable, Flagged_cases, as_tibble(superseded2), dups, dataPortalRates, add, add_21days, covidData, clean, Operational_metrics, PublicCOVID, PublicCOVID_mayor, SchoolCOVID, 
...
for(i in 1:length(filesToPrint)){
  ...
  if(grepl("csv", f)){
    cat("...")
    write.csv(d, file = paste0(outputLocation, "/", f), row.names = FALSE, sep = ",")}
  if(grepl("xlsx", f)){
    cat("...")
...
if(testing=="yes"){
  ...
  write.xlsx(pop_table_all, paste0(location, "/DAILY -- Case and Death Rates/", Sys.Date(), "(test)_Case and Death Rates.xlsx"), overwrite = T)
  write.xlsx(pop_table_all, paste0(location, internalReportFolder, "(test)Case and Death Rates.xlsx"), overwrite = T)
  write.xlsx(pop_table_all, paste0(mayorDashBox, "/(test)Case and Death Rates.xlsx"), overwrite = T)
  write.xlsx(pop_table_all, paste0(location, "/PUBLIC DASHBOARD FILES/", "(test)Case and Death Rates.xlsx"), overwrite = T)
  
  # Zip rate file
  write.xlsx(mayorDashZips, paste0(mayorDashBox, "/(test)Zip_Rates.xlsx"), overwrite = T) 
  write.xlsx(zipPopTable, paste0(location, internalReportFolder, "(test)Zip_Rates.xlsx"), overwrite = T)
  
  # Cases removed from analysis due to incorrect jurisdiction
  write.xlsx(Flagged_cases, paste0(location, "/DAILY -- Removed cases from other jurisdictions/", Sys.Date()," (test)Cases from other jurisdictions test.xlsx"), overwrite = T)
  
  # Duplicate deaths
  write.xlsx(dups, paste0(location, "/DAILY -- Duplicates from MDSS/", "(test)Duplicate_Death_Cases.xlsx"), overwrite = T) 
  
  # Death data
  write.table(deathsByDate, paste0(location, "/(test)Deaths by Date.csv"), sep = ",", row.names=FALSE)
  write.xlsx(deaths, file = paste0(location, "/(test)Deaths.xlsx"), sep="\t", row.names=FALSE, overwrite = T) 
  
  # The main full extract
  write.xlsx(covidData, paste0(location, "/(test)Full_extract_combined.xlsx"), overwrite = T) 
  write.xlsx(covidData, paste0(location, "/DAILY -- COVID Full Daily Extract Archives/", "(test)Full_extract_combined.xlsx"), overwrite = T) 
  
  # Cleaned full extract
  write.xlsx(clean, paste0(location, "/(test)Cleaned_Covid_Data.xlsx"), overwrite = T) 
  clean$PositiveCovidTestReportDates <- paste(clean$PositiveCovidTestReportDates) #flatten the list of tests to it can print CSV
  ...
  write.xlsx(PublicCOVID, paste0(location, "/(test)Public_COVID_Data.xlsx"), overwrite = T) 
  write.xlsx(PublicCOVID, paste0(location, "/DAILY -- Public Covid Data/",Sys.Date(),"_(test)Public_COVID_Data.xlsx"), overwrite = T) 
  write.xlsx(PublicCOVID, paste0(location, "/PUBLIC DASHBOARD FILES/", "_(test)Public_COVID_Data.xlsx"), overwrite = T) 
  
  # Mayor dashboard files
  write.xlsx(PublicCOVID_mayor, paste0(mayorDashBox, "/(test)Public Covid Mayor.xlsx"), overwrite = T) 
  write.xlsx(PublicCOVID_mayor, paste0(location, "/DAILY -- Public Covid Mayor/", Sys.Date(), "(test)Public Covid Mayor.xlsx"), overwrite = T) 
  ...
  if(overwrite=="yes"){
# Rate file for demographics
    write.xlsx(pop_table_all, paste0(location, "/DAILY -- Case and Death Rates/", Sys.Date(), "_Case and Death Rates.xlsx"), overwrite = T)
    write.xlsx(pop_table_all, paste0(location, internalReportFolder, "Case and Death Rates.xlsx"), overwrite = T)
    write.xlsx(pop_table_all, paste0(mayorDashBox, "/Case and Death Rates.xlsx"), overwrite = T)
    write.xlsx(pop_table_all, paste0(location, "/PUBLIC DASHBOARD FILES/", "Case and Death Rates.xlsx"), overwrite = T)
    
    # Zip rate file
    write.xlsx(mayorDashZips, paste0(mayorDashBox, "/Zip_Rates.xlsx"), overwrite = T) 
    write.xlsx(zipPopTable, paste0(location, internalReportFolder, "Zip_Rates.xlsx"), overwrite = T)
    
    # Cases removed from analysis due to incorrect jurisdiction
    write.xlsx(Flagged_cases, paste0(location, "/DAILY -- Removed cases from other jurisdictions/", Sys.Date()," Cases from other jurisdictions test.xlsx"), overwrite = T)
    
    # Duplicate deaths
    write.xlsx(dups, paste0(location, "/DAILY -- Duplicates from MDSS/", "Duplicate_Death_Cases.xlsx"), overwrite = T) 
    
    # Death data
    write.table(deathsByDate, paste0(location, "/Deaths by Date.csv"), sep = ",", row.names=FALSE)
    write.xlsx(deaths, file = paste0(location, "/Deaths.xlsx"), sep="\t", row.names=FALSE, overwrite = T) 
    
    # The main full extract
    write.xlsx(covidData, paste0(location, "/Full_extract_combined.xlsx"), overwrite = T) 
    write.xlsx(covidData, paste0(location, "/DAILY -- COVID Full Daily Extract Archives/", Sys.Date(), "_Full_extract_combined.xlsx"), overwrite = T) 
    
    # Cleaned full extract
    write.xlsx(clean, paste0(location, "/Cleaned_Covid_Data.xlsx"), overwrite = T) 
    clean$PositiveCovidTestReportDates <- paste(clean$PositiveCovidTestReportDates) #flatten the list of tests to it can print CSV
    ...
    #Mayor operational metrics
    write.xlsx(Operational_metrics, paste0(mayorDashBox, "/Operational metrics.xlsx"), overwrite = T) # for the mayor dashboard
    
    # Public dashboard files
    write.xlsx(PublicCOVID, paste0(location, "/Public_COVID_Data.xlsx"), overwrite = T) 
    write.xlsx(PublicCOVID, paste0(location, "/DAILY -- Public Covid Data/",Sys.Date(),"_Public_COVID_Data.xlsx"), overwrite = T) 
    write.xlsx(PublicCOVID, paste0(location, "/PUBLIC DASHBOARD FILES/", "Public_COVID_Data.xlsx"), overwrite = T) 
    
    # Mayor dashboard files
    write.xlsx(PublicCOVID_mayor, paste0(mayorDashBox, "/Public Covid Mayor.xlsx"), overwrite = T) 
    write.xlsx(PublicCOVID_mayor, paste0(location, "/DAILY -- Public Covid Mayor/", Sys.Date(), "Public Covid Mayor.xlsx"), overwrite = T) 
    
    # Occupations list
    write.table(occupations, file=paste0(location, "/Occupations_list.txt"), sep="|", row.names = FALSE)
    write.table(occupations_short, file=paste0(location, "/Occupations_list_short.txt"), sep="|", row.names = FALSE)
